...
export async function sendForgotPasswordEmail(
  ...
  await sendOrThrow({
    from: FROM,
    to,
    subject: "Reset Password BudgetIn",
    html: `
<!DOCTYPE html>
<html lang="id">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>
        ...
        <table width="480" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;border:1px solid #e5e7eb;overflow:hidden;">
          ...
          <tr>
            <td style="padding:32px;">
              <p style="margin:0 0 8px;font-size:22px;font-weight:700;color:#0f172a;">Hai, ${safeName}!</p>
              <p style="margin:0 0 24px;font-size:15px;color:#475569;line-height:1.6;">
                Kami menerima permintaan untuk mereset password akun BudgetIn kamu. Klik tombol di bawah untuk membuat password baru.
              </p>
              <table cellpadding="0" cellspacing="0" style="margin:0 0 24px;">
                <tr>
                  <td style="background:#0f172a;border-radius:8px;">
              ...
              <p style="margin:0;font-size:12px;color:#94a3b8;border-top:1px solid #f1f5f9;padding-top:16px;">
                Link ini berlaku selama <strong>1 jam</strong>. Jika kamu tidak meminta reset password, abaikan email ini — password kamu tidak akan berubah.
              </p>
            </td>
          </tr>
        </table>
...
export async function sendFamilyInviteEmail(params: {
  ...
  const { to, inviterName, familyName, displayRole, token } = params;
  const link = `${APP_URL}/family/join?token=${token}`;
  const safeInviter = escapeHtml(sanitizeName(inviterName));
  const safeFamily = escapeHtml(familyName);
  const safeRole = displayRole ? escapeHtml(displayRole) : null;

  await sendOrThrow({
    from: FROM,
    to,
    subject: `${inviterName} mengundangmu ke ${familyName} di BudgetIn`,
    html: `
<!DOCTYPE html>
<html lang="id">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>
      ...
      <td align="center">
        <table width="480" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;border:1px solid #e5e7eb;overflow:hidden;">
          <tr>
            <td style="background:#0f172a;padding:24px 32px;">
              <p style="margin:0;color:#f8fafc;font-size:20px;font-weight:700;letter-spacing:-0.5px;">BudgetIn</p>
              <p style="margin:4px 0 0;color:#94a3b8;font-size:12px;">Mode Keluarga</p>
            </td>
          </tr>
          <tr>
            <td style="padding:32px;">
              <p style="margin:0 0 8px;font-size:22px;font-weight:700;color:#0f172a;">Undangan Keluarga 👨‍👩‍👧</p>
              <p style="margin:0 0 24px;font-size:15px;color:#475569;line-height:1.6;">
                <strong>${safeInviter}</strong> mengundangmu bergabung ke keluarga <strong>${safeFamily}</strong>${safeRole ? ` sebagai <strong>${safeRole}</strong>` : ""} di BudgetIn.
              </p>
              <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:14px 18px;margin:0 0 24px;">
                <p style="margin:0;font-size:13px;color:#92400e;line-height:1.5;">
                  Dengan bergabung, transaksimu akan terlihat di <strong>tampilan Keluarga</strong> untuk anggota lain (kondisi keuangan gabungan). Buku pribadimu tetap milikmu.
                </p>
              </div>
              <table cellpadding="0" cellspacing="0" style="margin:0 0 24px;">
                <tr>
              ...
              <p style="margin:0 0 8px;font-size:12px;color:#94a3b8;">Atau salin link ini ke browser kamu:</p>
              <p style="margin:0 0 24px;font-size:11px;color:#64748b;word-break:break-all;">
                <a href="${link}" style="color:#3b82f6;">${link}</a>
              </p>
              <p style="margin:0;font-size:12px;color:#94a3b8;border-top:1px solid #f1f5f9;padding-top:16px;">
                Link ini berlaku selama <strong>7 hari</strong>. Jika kamu tidak mengenal pengirim undangan, abaikan email ini.
              </p>
            </td>
          </tr>
        </table>
